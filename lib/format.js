 ...
 */
exports.getAddressPCA = async (useMall) => {
  const res = await Province.findAll({
    include: [{ model: City,
      include: [{ model: Area }] }] })

  // 特殊处理：直辖市，直接替换成直辖市名称（仅仅用于useMall）
  const mcity = '市辖区'
  let mcityTime = 0
  // 特殊处理：重庆市，合并县和直辖市（仅仅用于useMall）
  const chq = '重庆市'
  // 特殊处理：省直辖县级行政区划和自治区直辖县级行政区划（仅仅用于useMall）
  const marea = ['省直辖县级行政区划', '自治区直辖县级行政区划']
  let mareaTime = 0
  ...
  const rC = _.map(res, p => {
    ...
    pd.children = _.map(p.cities, c => {
      ...
      if (useMall && marea.indexOf(cd.name) !== -1) {
        mareaArray = mareaArray.concat(cd.children)
      }
    ...
    if (mareaArray.length) {
      mareaObject[pd.name] = mareaArray
      _.remove(pd.children, t => marea.indexOf(t.name) !== -1)
      for (let value of marea) {
        delete r[pd.name][value]
      }
    }
  ...
  for (let name in mareaObject) {
    mareaTime++
    const item = _.find(rC, t => t.name === name)
    for (let value of mareaObject[name]) {
      const streets = await Street.findAll({ where: { areaCode: value.code } })
      value.children = _.map(streets, s => {
        const sd = s.dataValues
        sd.name = sd.name.replace('办事处', '')
        return _.pick(sd, cField)
      })
      if (r[name]) {
        r[name][value.name] = _.map(value.children, i => i.name)
      }
  ...
  if (useMall) {
    const chongqing = _.find(rC, t => t.name === chq)
    const chongqing2 = _.find(chongqing.children, t => t.name === chq)
    for (let item of chongqing.children) {
      if (item.name !== chq && chongqing2) {
        chongqing2.children = chongqing2.children.concat(item.children)
        r[chq][chq] = r[chq][chq].concat(_.map(item.children, i => i.name))
        delete r[chq][item.name]
      }
    }
    _.remove(chongqing.children, t => t.name !== chq)
  }

  if (!mcityTime || !mareaTime) {
    throw new Error('third data changed')
  }
